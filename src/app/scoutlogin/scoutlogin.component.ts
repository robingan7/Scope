...
export class ScoutloginComponent implements OnInit {
  ...
  private dataTypes2019 = {
    ...
    cargopickupspeed: {
      name: "cargopickupspeed",
      editPit: ""
    }, 
    hatchpickupspeed: {
      name: "hatchpickupspeed",
      editPit: ""
    }, 
    climbability: {
      name: "climbability",
    ...
    totalautopoint: {
      rankperference: false,
      name: "totalautopoint",
      fullName: "Total Auto Pnts",
      type: "Autonomous"
    }, 
    autohatch: {
      rankperference: false,
      name: "autohatch",
      fullName: "Hatch in Auto",
      type: "Autonomous"
    },
    autocargo: {
      rankperference: false,
      name: "autocargo",
      fullName: "Cargo in Auto",
      type: "Autonomous"
    },
    exitplatform:{
      rankperference: false,
      name: "exitplatform",
      fullName: "Exit Plaform pnts",
      type: "Autonomous",
      editMatch: ""
    },
    totalcargo: {
      rankperference: false,
      name: "totalcargo",
      fullName: "Total Cargo",
      type: "Teleop"
    },
    totalhatch: {
      rankperference: false,
      name: "totalhatch",
      fullName: "Total Hatch",
      type: "Teleop"
    },
    cargolvl1: {
      rankperference: false,
      name: "cargolvl1",
      fullName: "Cargo Level1",
      type: "Teleop",
      editMatch: ""
    },
    cargolvl2: {
      ...
      if (this.scoutb) {
        ...
        if (this.scoutb) {
          this.Auth.getPitForm(this.name, this.teamnumber).subscribe(data => {
            ...
            if (this.lengthh == 0) {
              //this.loadRankOpt(); run this for 2020 game
              ...
              this.Auth.getRankPerference(this.name, this.teamnumber, this.role).subscribe(data2 => {
                ...
                if (this.currentOption[0] != ""){
                  for (var ele in this.currentOption) {
                      this.dataTypes2019[this.currentOption[ele]].rankperference = true;
                  }
                  this.currentOptionA = data2.m.split("/")
                  for (var ele in this.currentOptionA) {
                    this.dataTypes2019[this.currentOptionA[ele]].rankperference = true
                  }
                  ...
                  for (var i = 0; i < optionsVar.length; i++) {
                    if (!this.contains(selectedoptionsVarlist, optionsVarlist[i].substring(0, optionsVar[i].value.length - 1))) {
                      this.dataTypes2019[optionsVar[i].value.substring(0, optionsVar[i].value.length - 1)].rankperference = false
                    } else {
                      ...
                      this.dataTypes2019[optionsVar[i].value.substring(0, optionsVar[i].value.length - 1)].rankperference = true
  ...
  //----load rankOpt----copy and paste to scoutinglogin
  /*
  loadRankOpt(){
    let rankOptTotal = "";
    let rankOptA = "";

    for (let [key, value] of Object.entries(this.dataTypes2019)){
      if (value.hasOwnProperty("rankperference")) {
        rankOptTotal += '<option *ngIf="!dataTypes2019.' + value.name + '.rankperference" value="' + value.name + '">' + value.fullName + 
                        '</option><option *ngIf="dataTypes2019.' + value.name + '.rankperference" value="' + value.name + '" selected>' + value.fullName + '</option>';
        rankOptA += '<option *ngIf="!dataTypes2019.' + value.name + '.rankperference" value="' + value.name + 'A">' + value.fullName +
          '</option><option *ngIf="dataTypes2019.' + value.name + '.rankperference" value="' + value.name + 'A" selected>' + value.fullName + '</option>';
      }
    }

    document.querySelector("#rankOpt").innerHTML = rankOptTotal;
    document.querySelector("#rankOptA").innerHTML = rankOptA;
  }*/
   ...
   */
  getPitTask(isDelete: Boolean, str = "") {
  ...
  addTask(type: any) {
    if (type == "match") {
      ...
      if (end_val == 0 || start_val == 0 || pos == "P" || this.name_addTask == "For who?") {
        this.addErrorC = this.responseError[1];
      } else if (this.currentRegionMainId == 0) {
        this.addErrorC = this.responseError[2];
      } else if (document.querySelector("#status_warn").className == "badge badge-pill badge-danger") {
        this.addErrorC = this.responseError[3];
      } else {
        this.addErrorC = this.responseError[0];
  ...
  editTask(e: any, type: string) {
    ...
    if (type == "match") {
      ...
      if (end_val == 0 || start_val == 0 || posi == "P") {
        ...
        this.Auth.editTaskMatch(this.teamnumber, this.name, name, JSON.stringify(this.team_member_task[this.name_addTask])).subscribe(data => {
          this.addErrorC = this.responseError[4];
          this.getTeamMemberTask();
        });
      }
    } else if (type == "pit") {
  ...
  getSelectedTeamPerference() {
    this.Auth.getSelectedTeamPerference(this.name, this.teamnumber, this.role).subscribe(data => {
      ...
      this.updateAndGetSelectTeamElement(true);
    });
  }

  updateAndGetSelectTeamElement(isFirstTime:boolean) {
    let ev = "";
    if(isFirstTime){
      ev = this.selctedPerference;
    } else {
      ev = (<HTMLSelectElement>document.querySelector("#selectTeamElement")).value
    }
    let type = ev.split("_")[0];
    let isTotal = ev.split("_")[1] == "Total";

    if (ev == 'General_Ability') {
      return;
    } else{
      this.selectTeamOption = ["isSelect"];
      this.selectTeamOptionInner = ["isSelect"];
      for (let [key, value] of Object.entries(this.dataTypes2019)) {
        if (this.dataTypes2019[key].type == type){
          if(isTotal){
            this.selectTeamOption.push(value.name);
          } else {
            this.selectTeamOption.push(value.name + 'A');
          }
          this.selectTeamOptionInner.push(this.dataTypes2019[key].fullName);
        }
      }
    }
    this.getSelectedTeam(this.selectTeamOption, this.selectTeamOptionInner)
  ...
  setcurrentRegionMain(e: any) {
    if (this.managerb) {
      this.currentRegionMain = e.target.currentevent.value
  ...
  loadrank() {
    ...
    for (var i = 0; i < optionsVar.length; i++) {
      if (!this.contains(selectedoptionsVarlist, optionsVarlist[i])) {
        this.dataTypes2019[optionsVar[i].value].rankperference = false;
      } else {
        this.currentOptionA.push(String(optionsVar[i].value) + "A");
        this.currentOptionInnerA.push(optionsVar[i].innerText);
        this.currentOption.push(optionsVar[i].value);
        this.currentOptionInner.push(optionsVar[i].innerText);
        this.dataTypes2019[optionsVar[i].value].rankperference = true;
      }
    }
  ...
  addhatchlvl3Edit(b: boolean) {
    ...
    var edit_tem_value = parseInt((<HTMLSelectElement>document.querySelector('#addhatchlvl3Edit')).value)
    this.Auth.updateMatchEdit("hatchlvl3", edit_tem_value, this.dataTypes2019.id.editMatch).subscribe(data => {});
  }
  ...
  addhatchlvl2Edit(b: boolean) {
    ...
    var edit_tem_value = parseInt((<HTMLSelectElement>document.querySelector('#addhatchlvl2Edit')).value)
    this.Auth.updateMatchEdit("hatchlvl2", edit_tem_value, this.dataTypes2019.id.editMatch).subscribe(data => {});
  }
  ...
  addhatchlvl1Edit(b: boolean) {
    ...
    var edit_tem_value = parseInt((<HTMLSelectElement>document.querySelector('#addhatchlvl1Edit')).value)
    this.Auth.updateMatchEdit("hatchlvl1", edit_tem_value, this.dataTypes2019.id.editMatch).subscribe(data => {});
  ...
  addcargolvl3Edit(b: boolean) {
    ...
    var edit_tem_value = parseInt((<HTMLSelectElement>document.querySelector('#addcargolvl3Edit')).value)
    this.Auth.updateMatchEdit("cargolvl3", edit_tem_value, this.dataTypes2019.id.editMatch).subscribe(data => {});
  ...
  addcargolvl2Edit(b: boolean) {
    ...
    var edit_tem_value = parseInt((<HTMLSelectElement>document.querySelector('#addcargolvl2Edit')).value)
    this.Auth.updateMatchEdit("cargolvl2", edit_tem_value, this.dataTypes2019.id.editMatch).subscribe(data => {});
  }
  ...
  addcargolvl1Edit(b: boolean) {
    ...
    var edit_tem_value = parseInt((<HTMLSelectElement>document.querySelector('#addcargolvl1Edit')).value)
    this.Auth.updateMatchEdit("cargolvl1", edit_tem_value, this.dataTypes2019.id.editMatch).subscribe(data => {});
  }
  ...
  addhatchshipEdit(b: boolean) {
    ...
    var edit_tem_value = parseInt((<HTMLSelectElement>document.querySelector('#addhatchshipEdit')).value)
    this.Auth.updateMatchEdit("hatchship", edit_tem_value, this.dataTypes2019.id.editMatch).subscribe(data => {});
  }
  ...
  addcargoshipEdit(b: boolean) {
    ...
    var edit_tem_value = parseInt((<HTMLSelectElement>document.querySelector('#addcargoshipEdit')).value)
    this.Auth.updateMatchEdit("cargoship", edit_tem_value, this.dataTypes2019.id.editMatch).subscribe(data => {});
  }
  addeditdroppieceEdit(b: boolean) {
    ...
    var edit_tem_value = parseInt((<HTMLSelectElement>document.querySelector('#addeditdroppieceEdit')).value)
    this.Auth.updateMatchEdit("defensedrop", edit_tem_value, this.dataTypes2019.id.editMatch).subscribe(data => {});
  }
  addeditblockEdit(b: boolean) {
    ...
    var edit_tem_value = parseInt((<HTMLSelectElement>document.querySelector('#addeditblockEdit')).value)
    this.Auth.updateMatchEdit("defenseblock", edit_tem_value, this.dataTypes2019.id.editMatch).subscribe(data => {});
  }
  addeditdrophatchEdit(b: boolean) {
    ...
    var edit_tem_value = parseInt((<HTMLSelectElement>document.querySelector('#addeditdrophatchEdit')).value)
    this.Auth.updateMatchEdit("hatchdrop", edit_tem_value, this.dataTypes2019.id.editMatch).subscribe(data => {});
  }
  addeditdropcargoEdit(b: boolean) {
    ...
    var edit_tem_value = parseInt((<HTMLSelectElement>document.querySelector('#addeditdropcargoEdit')).value)
    this.Auth.updateMatchEdit("cargodrop", edit_tem_value, this.dataTypes2019.id.editMatch).subscribe(data => {});
  }
  ...
  setEditingToFalse() {
    ...
    this.Auth.updateTotalVal(this.teamnumber, this.dataTypes2019.region.editMatch, this.dataTypes2019.teamnumber.editMatch).subscribe(data => {});
  }

  updateMatchEdit(e:any) {
    e.preventDefault();
    const target = e.target;
    if (target.id == 'region' || target.id == 'matchnumber' || target.id == 'teamnumber') {
      this.editingThethree = true
    }
    this.Auth.updateMatchEdit(target.id, target.value, this.dataTypes2019.id.editMatch).subscribe(data => {
      this.Auth.updateTotalVal(this.teamnumber, this.dataTypes2019.region.editMatch, this.dataTypes2019.teamnumber.editMatch).subscribe(data => {
      });

      if (target.id == 'teamnumber') {
        this.Auth.updateTotalVal(this.teamnumber, this.dataTypes2019.region.editMatch, target.value).subscribe(data => {});
      } else if (target.id == 'region') {
        this.Auth.updateTotalVal(this.teamnumber, target.value, this.dataTypes2019.teamnumber.editMatch).subscribe(data => {});
      }
    });
  }
  ...
  updatePitEdit(e:any) {
    ...
    this.Auth.updatePitEdit(target.id, result, this.dataTypes2019.id.editPit).subscribe(data => {});
  }

  getEditDataMatchHelper(data:any) {
    for (let [key, value] of Object.entries(this.dataTypes2019)) {
      if (value.hasOwnProperty("editMatch")) {
        this.dataTypes2019[key].editMatch = data.m[0][value.name];
      }
    }
    this.editMatchLoader = false;
  }
  getEditDataMatch(e:any) {
    ...
    if (inner.length == 3) {
      this.Auth.getEditDataMatch(this.teamnumber, this.history_person, inner[0], inner[1], inner[2]).subscribe(data => {
        this.getEditDataMatchHelper(data);
      });
    } else {
  ...
  getEditDataMatchPit(e:any) {
    this.resetSensor_Auto = false;
    this.editingTeamnumber = false;
    this.editPitLoader = true;
    const inner = e.target.innerText;
    this.dataTypes2019.autochoice.editPit = [];
    this.dataTypes2019.sensor.editPit = [];
    ...
    this.Auth.getEditDataMatchPit(this.teamnumber, this.history_person, inner).subscribe(data => {
      for (let [key, value] of Object.entries(this.dataTypes2019)) {
        if (value.hasOwnProperty("editPit")) {
          let editdatastringPit = value.name;
          if (editdatastringPit == "sensor" || editdatastringPit == "autochoice") {
            this.dataTypes2019[editdatastringPit].editPit = data.m[0][editdatastringPit].split("/");
          } else {
            this.dataTypes2019[editdatastringPit].editPit = data.m[0][editdatastringPit];
          }
        }
        if (this.dataTypes2019.autochoice.editPit[0] == "" && this.dataTypes2019.autochoice.editPit.length == 1) {
          this.dataTypes2019.autochoice.editPit = ["empty"];
        }
        if (this.dataTypes2019.sensor.editPit[0] == "" && this.dataTypes2019.sensor.editPit.length == 1) {
          this.dataTypes2019.sensor.editPit = ["empty"];
        }
  ...
  openHistory(e: any) {
    e.preventDefault();
    const inner = 'a' + e.target.innerText.split(" ")[0] + "_match";
  ...
  openHistoryPit(e:any) {
    e.preventDefault();
    const inner = 'a' + e.target.innerText.split(" ")[0] + "_pit"
  ...
  getEventVP3(e: any) {
    this.Auth.getAPI3("events/" + this.currentYear).subscribe(data => {
      ...
      if (e.target.value == "Houston") {
      ...
      } else if (e.target.value == "Offseason") {
        for (var ele in data) {
        ...
        for (var ele in data) {
          if (data[ele].week == e.target.value - 1) {
            selectedevent.push({
              long: data[ele].name,
              short: data[ele].short_name
            });
  ...
  sendcurrentPit(e: any) {
    e.preventDefault();
    this.currentPitReceiver = String(e.target.value);
  ...
  sendcurrentMatchTeam(e: any) {
    e.preventDefault();
    this.currentMatchTeam = e.target.value;
  }

  sendcurrentMatchNumber(e: any) {
    e.preventDefault();
    this.currentMatchNumber = e.target.value;
  }

  sendcurrentRegion(e: any) {
    e.preventDefault();
  ...
  setCurrentImageTitle(e: any) {
    e.preventDefault();
    this.currentImageTitle = e.target.value;
  }

  setCurrentImage(e: any) {
    e.preventDefault();
...
/* notused
use in current task page, replaced taskOut2
taskOut(e: any){
  e.preventDefault();
  var id = e.target.id;
...
-------was in signout method------
  ...
  editdata = {
    id: -1, region: "none", matchnumber: "0", teamnumber: "0000", alliance: "blue", exitplatform: 0, autohatch: 0, autocargo: 0,
    ...
    if (ev == 'General_Ability') {
      ...
      for (let [key, value] of Object.entries(this.dataTypes2019)) {
        if(value.type == type){
          ...
          this.selectTeamOptionInner.push(value.fullName);
        }
      }
    }
    this.getSelectedTeam(this.selectTeamOption, this.selectTeamOptionInner)
