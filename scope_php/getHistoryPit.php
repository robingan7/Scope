...
if(isset($_POST) && !empty($_POST)) {
    ...
    $temmatchele="";
    $bs="";
    $sql2="SELECT * FROM teams WHERE scout_team='$scout_team' AND scout_name='$scout_name' ORDER BY `teams`.`date2` ASC;";
    ...
    while($rows= mysqli_fetch_assoc($result2)){
        if($rows['date2']!=$tem){
            ...
            $sql3="SELECT * FROM teams WHERE scout_team='$scout_team' AND scout_name='$scout_name' AND date2='$tem' ORDER BY `teams`.`date2` ASC;";
            $result3=mysqli_query($conn, $sql3);
            while($rows2= mysqli_fetch_assoc($result3)){
                $temmatchele=$temmatchele.(String)$rows2['teamnumber']."/";
            }
